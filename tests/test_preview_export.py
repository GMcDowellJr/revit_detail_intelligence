...
from dse.revit_api.preview_export import _source_scope_hash, get_cached_view_preview
...
def test_get_cached_view_preview_uses_doc_scope(tmp_path):
    preview_root = tmp_path / "previews"
    preview_root.mkdir(parents=True, exist_ok=True)

    scoped_a = preview_root / "view_123__doc_{}.png".format(_source_scope_hash("doc-a", None))
    scoped_b = preview_root / "view_123__doc_{}.png".format(_source_scope_hash("doc-b", None))
    _write_png(str(scoped_a), 128, 64)
    _write_png(str(scoped_b), 128, 64)

    cfg = {"preview_root": str(preview_root), "preview_longest_side": 120}
    assert (
        get_cached_view_preview(123, cfg, source_doc_id="doc-a")
        == str(scoped_a)
    )
    assert (
...
def test_source_scope_hash_uses_doc_id_and_name():
    base = _source_scope_hash("same-id", "Doc A")
    changed_name = _source_scope_hash("same-id", "Doc B")
    changed_id = _source_scope_hash("other-id", "Doc A")
